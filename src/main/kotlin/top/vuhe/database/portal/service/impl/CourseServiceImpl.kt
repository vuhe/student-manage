...
class CourseServiceImpl: ServiceImpl<CourseMapper, Course>(), CourseService {
    ...
    override fun addCourse(course: Course): ApiResponse<*> {
        // id 由数据库设定
        // 此处 id 置空
        val ans = save(course.copy(id = null))
        // 检验结果
        return if (ans) {
    ...
    override fun modifyCourse(course: Course): ApiResponse<*> {
        val ans = updateById(course)
        // 检验结果
        return if (ans) {
            ApiResponse.ofSuccess()
        } else {
